...
async def ocr_book_cover(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if raw_text:
        ...
        url = (
            f"https://www.googleapis.com/books/v1/volumes?q={clean_query}&maxResults=1"
        )
        try:
            ...
            metadata = None
    return BookOCRResponse(
        title=title, author=author, raw_text=raw_text, metadata=metadata
    )
...
async def add_book(payload: BookCreate, db: Session = Depends(get_db)):
    metadata = await fetch_metadata(
        str(payload.url)
    )  # Fetch title/description from the URL
